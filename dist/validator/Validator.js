...
class Validator {
    ...
    isValid(body, showLogAttributeIsNotValid) {
    ...
    verifyIsValidAndEmmitValidationError(body) {
        const check = this.verify(body);
        if (!check.isValid)
            throw check.error;
        return true;
    }
    verifyIsValidAndEmmitHttpError(body) {
        const check = this.verify(body);
        if (!check.isValid) {
    ...
    adapterToHttpMiddleware(httpRequestOption) {
        return (httpRequest) => {
